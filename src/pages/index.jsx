			...
			<main className='w-full h-screen grid grid-rows-web-layout'>
				<header className='w-full flex justify-between items-center px-6 py-4'>
					...
					<a
						...
						className='px-4 py-2 flex items-center gap-2 rounded-md bg-white border border-slate-200 font-medium text-sm'
					>
				...
				<section className='w-full h-full grid grid-rows-landing-layout'>
					<header className='flex flex-col items-center justify-center p-6 text-center lg:py-24'>
						<h1 className='font-bold text-3xl lg:text-6xl text-center text-slate-900 lg:max-w-[20ch]'>
							La plataforma de videoconferencias gratuita
						</h1>
						<p className='text-lg lg:text-xl mt-4 text-slate-500'>
					...
					<main className='flex items-center justify-center px-4'>
						<BrowserFrame>
							{isAuthLoading ? (
								...
								<>
									<section className='h-full px-3 py-4 flex flex-col gap-4 lg:justify-center'>
										<UserCard user={user} onLogout={logout} />
										<section className='flex flex-col gap-6 bg-white border border-slate-200 rounded-md shadow-2xl shadow-slate-200 p-6 lg:w-[384px] lg:mx-auto'>
											{roomName === '' && (
								...
								<section className='h-full p-6 lg:p-0 flex items-center justify-center'>
									<section className='flex flex-col gap-6 bg-white border border-slate-200 rounded-md shadow-2xl shadow-slate-200 p-6 max-w-xs lg:max-w-sm'>
										<p className='font-semibold text-center'>Unete a Meetapp</p>
										<button
											onClick={login}
											className='px-6 py-3 rounded-md text-sm font-medium bg-slate-800 text-slate-100 flex items-center justify-center gap-2 shadow-xl shadow-slate-400/30 hover:bg-slate-700 transition-colors'
										>
							...
							{/* <main className='h-full p-6 lg:p-0 flex items-center justify-center'>
							{isAuthLoading || isLoading ? (
								<Spinner className='text-slate-500' />
							) : user ? (
								<section className='flex flex-col gap-6 bg-white border border-slate-200 rounded-md shadow-2xl shadow-slate-200 p-6 transition-height'>
									<article className='flex lg:flex-col justify-center items-center gap-2'>
				...
				<footer className='w-full flex justify-center items-center px-6 py-3 border-t border-t-slate-200'>
...
function UserCard({ user, onLogout }) {
	return (
		<article className='p-4 rounded-md border border-slate-200 grid grid-cols-card-user-layout gap-4 bg-white lg:w-[384px] lg:mx-auto'>
			<figure className='w-10 h-10 rounded-full overflow-hidden relative'>
				<Image src={user.image} alt={user.name} layout='fill' />
			</figure>
			<section className='whitespace-nowrap overflow-hidden text-ellipsis'>
				...
				<p className='font-medium text-xs lg:text-sm text-slate-500 self-center whitespace-nowrap overflow-hidden text-ellipsis'>
					{user.email}
				</p>
			</section>
			<button
...
function ErrorNotification({ content, onClose }) {
	return (
		<section className='flex items-center gap-3 text-sm font-medium px-4 py-3 rounded-md bg-white border border-slate-200 text-slate-800 shadow-2xl shadow-slate-200'>
			...
			<button
				onClick={onClose}
				className='self-start text-slate-400 hover:text-slate-600 hover:bg-slate-50 px-1 py-[2px] rounded-md'
			>
				<XIcon size={16} />
			</button>
		</section>
